...
- [ ] Add event thumbnails, clips, class, confidence, zone, and count. (The private event schema, authenticated ingest/read API, source-segment thumbnail extraction, bounded clip routes, retention cleanup, and motion-gated pipeline now exist. Offline replay of a real archived segment read 900 frames and produced three confirmed `person` events at 0.888–0.909 confidence; those events were ingested through the protected endpoint, rendered as three thumbnails and three bounded clips, and verified in the authenticated dashboard. A guarded source-linked Pi detector hook is now deployed but remains disabled in the active archive service; labeled accuracy and a reviewed canary remain before checking this item.)
- [ ] Display live boxes and counts without baking overlays into original evidence video. (Added and tested a non-destructive overlay renderer and pipeline output; generated a real 10-second H.264 proof clip from a Pi segment; the Pi MJPEG relay now has a source-preserving, explicit `--enable-overlay` canary path deployed but disabled in the active service. A private 10-second sequential-frame canary rendered `person: 1` with a stable `#1` box at 0.68 confidence; live overlay acceptance remains gated on labeled accuracy.)
