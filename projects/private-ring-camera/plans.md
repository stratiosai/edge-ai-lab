...
- [x] Browse a timeline covering the available portion of the last 24 hours. (Authenticated UI verification showed the rolling timeline; after the production retention rollover on 2026-09-21 it retained 521 recordings spanning 23.97 hours, with the oldest expired content removed.)
...
- [x] Confirm recordings expire automatically after 24 hours. (The exact boundary is covered by an authenticated end-to-end test, and the production startup retention pass on 2026-09-21 removed 10 segments older than the cutoff; a fresh database query reported `expired_rows=0`, HTTPS returned 200, and all three Pi services remained active.)
...
**Active subtask:** continue Phase 5 detector preparation. Tailscale is installed and connected on the Mac, Pi, and operator phone; the camera services are reachable over the private Tailscale path and phone access was confirmed. Router UPnP/NAT-T hardening passed on 2026-09-21. Phase 5's detector candidates, motion-zone configuration, measurement manifest, Pi ONNX benchmarks, detector adapter, guarded Pi-agent hook, motion-gated pipeline, offline segment replay, IoU tracker, crossing-line logic, event metadata contract, labeled-accuracy CLI, condition-tagged private frame-label sampler, prediction sidecar, live-overlay canary path, privacy-mask wiring, and the required-condition environmental evaluator are documented under [`phase5/README.md`](phase5/README.md). A manually reviewed 12-frame controlled-indoor sample measures 14/14 true-positive object matches with zero false positives, misses, or duplicate predictions; one no-upload real-Pi canary produced two person suggestions at 0.567 and 0.491 confidence, the next quiet-segment canary produced zero events under the motion gate, and a closed 60-second daylight segment was replayed without modifying the source while producing a private derived overlay proof clip. The synthetic six-condition smoke matrix correctly failed acceptance; real environmental coverage and unattended live detection remain next.
...
- [x] Add configurable crossing lines and approach/departure direction. (Added normalized-coordinate crossing-line direction detection with a jitter deadband, one event per track, expiry reset, pipeline integration for confirmed tracks, guarded live-overlay CLI wiring, and tests; unattended enablement remains gated.)
- [ ] Configure a motion-activated vehicle-passing counter for a driveway/road view. (The guarded overlay now wires the motion gate, class-aware tracker, and optional normalized crossing line; car/truck/bus/motorcycle recognition, direction counts, and unattended enablement remain gated on real environmental accuracy.)
- [x] Add event thumbnails, clips, class, confidence, zone, and count. (The private event schema, authenticated ingest/read API, source-segment thumbnail extraction, bounded clip routes, retention cleanup, motion-gated pipeline, controlled indoor labeled baseline, and a no-upload real-Pi detector canary are implemented. Offline replay produced three authenticated dashboard events with thumbnails and bounded clips, and the Pi canary produced two source-linked person suggestions. Unattended detector enablement remains separately gated by the environmental-accuracy and live-acceptance items below.)
- [x] Display live boxes and counts without baking overlays into original evidence video. (The non-destructive overlay renderer and pipeline are tested; a real 10-second 1920×1080/15fps proof clip rendered `person: 1` with a stable `#1` box at 0.68 confidence, and the Pi MJPEG relay has an explicit source-preserving `--enable-overlay` canary path. The normal live service remains overlay-disabled while the separate environmental and unattended-acceptance gates remain open.)
