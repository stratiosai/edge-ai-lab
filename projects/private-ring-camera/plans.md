...
- [ ] Add event thumbnails, clips, class, confidence, zone, and count. (The private event schema, authenticated ingest/read API, source-segment thumbnail extraction, bounded clip routes, retention cleanup, motion-gated pipeline, controlled indoor labeled baseline, and a no-upload real-Pi detector canary now exist. Offline replay produced three authenticated dashboard events with thumbnails and bounded clips, and the Pi canary produced two source-linked person suggestions. This remains unchecked until real environmental accuracy is measured and the detector is reviewed for unattended use.)
