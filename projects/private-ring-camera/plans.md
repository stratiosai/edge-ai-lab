...
- [ ] Add event thumbnails, clips, class, confidence, zone, and count. (The private event schema, authenticated ingest/read API, source-segment thumbnail extraction, bounded clip routes, retention cleanup, and motion-gated pipeline now exist. Offline replay of the real private Pi segment read 900 frames and produced two distinct confirmed `person` tracks/events at 0.914 and 0.895 confidence in `controlled-test-area`; authenticated Pi ingest and source-linked thumbnail/clip verification remain.)
