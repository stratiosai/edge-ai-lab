...
- [ ] Display live boxes and counts without baking overlays into original evidence video. (Added and tested a non-destructive overlay renderer and pipeline output; generated a real 10-second H.264 proof clip from a Pi segment; the Pi MJPEG relay has a source-preserving, explicit `--enable-overlay` canary path deployed but disabled in the active service. A private sequential-frame canary rendered `person: 1` with a stable `#1` box at 0.68 confidence; broader environmental and unattended-acceptance gates remain.)
