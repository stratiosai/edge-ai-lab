...
- [ ] Add event thumbnails, clips, class, confidence, zone, and count. (The private event schema, authenticated ingest/read API, source-segment thumbnail extraction, bounded clip routes, and retention cleanup now exist; detector wiring and real event verification remain.)
- [ ] Display live boxes and counts without baking overlays into original evidence video. (Added and tested a non-destructive overlay renderer; live analysis-stream wiring remains.)
